...
export const AlertsNotifications: React.FC = () => {
  const { chokepoints, weatherStations, vehicles, addToast, isDisasterModeActive } = useLogistics();
  const { language, setLanguage, speakAlert, t } = useLanguage();
  ...
  const handleBroadcast = (e: React.FormEvent) => {
    ...
    addToast(priorityLevel === 'CRITICAL' ? 'CRITICAL' : 'SUCCESS', 'Alert Broadcasted', `Dispatched to all connected fleet telemetry units & authorities.`);
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
        ...
        {isDisasterModeActive && (
          <div className="px-3 py-1.5 rounded-xl bg-rose-600/90 text-white text-xs font-mono font-bold flex items-center space-x-2 shadow-glow-rose animate-pulse">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel-glow p-5 rounded-2xl border border-cyan-500/30 space-y-4 lg:col-span-2">
          <div className="flex items-center justify-between pb-2 border-b border-white/10">
            ...
            <span className="text-[10px] font-mono text-slate-400">Channels: WebSockets, Push, Radio/TTS</span>
          </div>

          <form onSubmit={handleBroadcast} className="space-y-4 text-xs">
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Alert Headline</label>
              <input
                type="text"
                required
                placeholder="e.g. Critical Bridge Closure at NH-29 - Alternateha Haflong Active"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block font-semibold text-slate-300 mb-1">Target Corridor / Jurisdiction</label>
                <select
              ...
              <div>
                <label className="block font-semibold text-slate-300 mb-1">Urgency Priority Level</label>
            ...
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Detailed Instructions / Diversion Notes</label>
              <textarea
                rows={3}
                required
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
              ...
              <button
                type="submit"
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-rose-500 hover:from-amber-400 hover:to-rose-400 text-slate-950 font-bold text-xs shadow-glow-amber flex items-center space-x-2 transition-all"
              >
        ...
        <div className="glass-panel p-5 rounded-2xl border border-white/10 space-y-4 flex flex-col justify-between">
          <div>
            <h3 className="text-xs uppercase font-bold text-teal-400 tracking-wider flex items-center space-x-2 pb-2 border-b border-white/10">
            ...
            <div className="space-y-3 mt-3 text-xs">
              <div className="p-3 rounded-xl bg-slate-900/70 border border-white/5 flex items-center justify-between">
                <div>
                  <div className="font-semibold text-white">Fleet Telemetry WS</div>
                  <div className="text-[10px] text-slate-400">{vehicles.length} Active Nodes Connected</div>
                </div>
              ...
              <div className="p-3 rounded-xl bg-slate-900/70 border border-white/5 flex items-center justify-between">
                <div>
                  <div className="font-semibold text-white">BRO / NDRF Alert Link</div>
                  <div className="text-[10px] text-slate-400">Direct Secure Dispatch</div>
                </div>
              ...
              <div className="p-3 rounded-xl bg-slate-900/70 border border-white/5 flex items-center justify-between">
                <div>
                  <div className="font-semibold text-white">Doppler Weather Feed</div>
                  <div className="text-[10px] text-slate-400">{weatherStations.length} Regional Stations</div>
                </div>
                <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                  SYNCED
                </span>
          ...
          <div className="p-3.5 rounded-xl bg-cyan-950/40 border border-cyan-500/20 text-[11px] text-slate-300 space-y-1">
            <div className="text-[10px] uppercase font-bold text-cyan-400">Broadcast Guidelines:</div>
      ...
      <div className="glass-panel p-5 rounded-2xl space-y-4 border border-white/10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-white/10">
          <div className="flex items-center space-x-2">
            <h3 className="text-xs uppercase font-bold text-slate-300 tracking-wider flex items-center space-x-2">
              <Filter className="w-4 h-4 text-cyan-400" />
              <span>Active Alert & Notification Log</span>
            </h3>
            <span className="text-xs text-slate-400 font-mono">({filteredAlerts.length} Logs)</span>
          </div>
                ...
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[9px] font-mono font-bold uppercase ${
                      ...
                      isWeath ? 'bg-blue-500/20 text-blue-300' :
                ...
                <div className="flex items-center space-x-2 sm:self-center shrink-0">
                  <button
                    onClick={() => {
                      ...
                      addToast('INFO', 'Voice Replayed', 'Replaying broadcast audio.');
                    }}
                    className="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-cyan-300 border border-cyan-500/20 text-xs font-semibold flex items-center space-x-1.5 transition-colors"
                    title="Play multilingual text-to-speech announcement"
                  >
                    <Volume2 className="w-4 h-4 text-cyan-400" />
                    <span className="hidden md:inline">Play TTS</span>
                  </button>
                </div>
              </div>
            );
